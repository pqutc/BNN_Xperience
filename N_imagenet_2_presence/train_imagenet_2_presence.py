...
net = Reactnet_1(num_classes=Targetnum, imagenet=True)
...
class Reactnet_2(nn.Module):

    def __init__(self, alpha=0.25, num_classes=1000, imagenet=True):
        super(Reactnet_2, self).__init__()
...
net = Reactnet_2(num_classes=Targetnum, imagenet=True)
net = nn.DataParallel(net).cuda()
max_test_acc = 0.
if Begin_epoch!=0:
    net.load_state_dict(torch.load(Savemodel_path + f'epoch{Begin_epoch-1}{Name_suffix}.h5'))
...
try:
    torch.save(Confusion_Matrix.cpu(), Record_path+"confusion_matrix_step2.pt")
    pd.DataFrame(Confusion_Matrix.cpu().numpy()).to_csv(Record_path+"confusion_matrix_step2.csv", index=False)
    with open("accuracy_step2.txt", "w") as f:
        f.write(f"Accuracy: {acc.item():.4f}\n")
except Exception as e:
    print(f"Erreur lors de la sauvegarde des résultats : {e}")